...
import { ILeague, IStaticProps, ITopScorePlayer, ITopScorePlayerClubResponse, ITopScorePlayerResponse } from '@/types/app-type'
...
const TopScoresComponent = ({ data }: IStaticProps<IDataFootball>) => {
        ...
        <>
                            ...
                            return (
                              <tr key={index} className="text-sm [&>td]:p-2 border-b border-[#eee] flex flex-col sm:table-row">
                                ...
                                <td className="text-left">
                                  <Link href={`/${stringToSlug(item.player_name)}/${item.player_id}/player`}>{item.player_name}</Link>
                                </td>
                                <td>{item.goals}</td>
                                <td>{item.penalty}</td>
                              </tr>
          ...
          {data?.topScore && Array.isArray(data.topScore) && data.topScore.length > 0 && (
            <table className="w-full text-center">
              <thead>
                <tr className="bg-[#edf2f7] text-sm [&>th]:p-2 w-full ">
                  <th>{t('match')}</th>
                  <th className="text-left">{t('player')}</th>
                  <th>{t('goals')}</th>
                  <th>{t('penalty')}</th>
                ...
                {data?.topScore.map((item, index) => {
                  return (
                    <tr key={index} className="text-sm [&>td]:p-2 border-b border-[#eee]">
                      ...
                      <td className="text-left">
                        <p>
                          <Link href={`/${stringToSlug(item.player_name)}/${item.player_id}/player`}>
      ...
      {pageType === 1 && leagues && (
        <>
          <table className="w-full text-center flex sm:table">
            ...
            {data.highest && data.highest?.items?.length > 0 && (
              <tbody className="flex flex-row sm:table-row-group overflow-auto">
...
export default TopScoresComponent
